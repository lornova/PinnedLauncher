...
**Current status: documentation only — no code yet.** The design is conditionally
accepted pending **spike S-3** (the go/no-go); implementation must not start before
S-3 passes. See [docs/implementation-plan.md](docs/implementation-plan.md) for phases
P0–P3 and the release train.
