#Requires -Version 7
# PinnedLauncher spike S-9 (throwaway): compiles the scratch tools into bin\
# using the newest installed Visual Studio C++ toolchain (x64). s9uia is a
# console tool (prints element dumps); s9target is a real windowed app.
...
$targets = @(
    @{ name = 's9uia';    subsystem = 'CONSOLE'; libs = 'ole32.lib oleaut32.lib' }
    @{ name = 's9target'; subsystem = 'WINDOWS'; libs = 'user32.lib' }
)
