#Requires -Version 7
# PinnedLauncher spike S-3 (throwaway): compiles the two scratch exes into bin\
# using the newest installed Visual Studio C++ toolchain (x64).
...
try {
    foreach ($name in 's3proxy', 's3selfaumid') {
        # vcvars64.bat invokes vswhere itself and expects it on PATH
        cmd /c "set `"PATH=$installerDir;%PATH%`" && `"$vcvars`" >nul && cl /nologo /O1 /W4 /EHsc /std:c++20 /DUNICODE /D_UNICODE `"$srcDir\$name.cpp`" /Fe:$name.exe /link /SUBSYSTEM:WINDOWS shell32.lib ole32.lib user32.lib"
